...
class TenantController {
    ...
    createTenant = async (req, res) => {
        const { owner_id, room_id, name, owner_name, total_members, merital_status, address_line_1, address_line_2, country_id, state_id, city_id, pin_code, adhaar_card_no, contact_name_1, contact_no_1, email_id_1, contact_name_2, contact_no_2, email_id_2, lease_start_date, lease_end_date, deposit_ammount } = req.body
    ...
    updateTenant = async (req, res) => {
        const { tenant_id, owner_id, room_id, name, owner_name, total_members, merital_status, address_line_1, address_line_2, country_id, state_id, city_id, pin_code, adhaar_card_no, contact_name_1, contact_no_1, email_id_1, contact_name_2, contact_no_2, email_id_2, lease_start_date, lease_end_date, deposit_ammount } = req.body
        console.log({ owner_id, name, owner_name, total_members, merital_status, address_line_1, address_line_2, country_id, state_id, city_id, pin_code, adhaar_card_no, contact_name_1, contact_no_1, email_id_1, contact_name_2, contact_no_2, email_id_2, lease_start_date, lease_end_date, deposit_ammount })
        const { profile_photo, adhaar_card_image, signature_photo } = req.files
        ...
        const updateTenant = await tenantFuncitons.updateTenant(tenantData)
        if (updateTenant) {
            return response.success('Get All updateTenant Successfully', updateTenant)
        } else {
            return response.fail('Get All updateTenant fail', [])
        }
    }
    ...
    getTenant = async (req, res) => {
        ...
        const getTenant = await tenantFuncitons.getTenant({ tenant_id })
        if (getTenant) {
            return response.success('Get All getTenant Successfully', getTenant)
        } else {
            return response.fail('Get All getTenant fail', [])
        }
    }
