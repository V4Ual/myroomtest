...
class RoomController {
  ...
  editRoomDetails = async (req, res) => {
    const { room_id, user_id, room_type_id, address_1, address_2, bethroom, room, about, monthly_amount, deposite_amount } = req.body
    const { room_cover_image } = req.files
    ...
    const editRoomDetails = await roomFunctions.editRoomDetails(addData)
    if (editRoomDetails) {
      return response.success('Edit Details Successfully', editRoomDetails)
    } else {
      return response.fail('Fail To Edit Details', [])
    }
